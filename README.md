...
For a configured harness family, authd matches the kernel process command name against the configured `executable_names` (`claude` or `codex`) through one best-effort family boundary. It independently validates the selected live process PID/start time, executable device/inode, target UID/GID, and a same-UID verified terminal origin; the harness must remain in that terminal session, with matching session and nonzero controlling TTY. Wayland/runtime environment and runtime-directory ownership are validated from the harness session, and the harness and terminal leader are rechecked for start-time stability. Authd does not canonicalize launcher files, so a launcher symlink retarget does not affect an already-running process. Claude `!` commands and ordinary Claude shell descendants share the Claude Agent generation; Codex uses its own family.
